const CheckBox = ({ label, value, checked, onChange }) => (
  <label className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all duration-200 select-none
    ${checked
      ? "border-blue-500 bg-blue-50 dark:bg-blue-900/40 text-blue-700 dark:text-slate-100 font-semibold shadow-sm"
      : "border-blue-100 dark:border-gray-700 bg-white dark:bg-gray-900/60 text-slate-600 dark:text-slate-300 hover:border-blue-300 dark:hover:border-gray-600 hover:bg-blue-50/50 dark:hover:bg-gray-800/30"}`}>
    <span className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all duration-200
      ${checked ? "bg-blue-500 border-blue-500" : "border-blue-300 dark:border-gray-600 bg-white dark:bg-gray-900"}`}>
...
const RadioBtn = ({ label, value, checked, onChange, name }) => (
  <label className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all duration-200 select-none
    ...
    <span className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all duration-200
      ${checked ? "border-blue-500" : "border-blue-300 dark:border-gray-600 bg-white dark:bg-gray-900"}`}>
...
const InputField = ({ label, type = "text", value, onChange, placeholder, required }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <input
      ...
      className="w-full px-4 py-3 rounded-xl border-2 border-blue-100 dark:border-gray-700 bg-white dark:bg-gray-900/60 text-slate-700 dark:text-slate-100 text-sm
        placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100 dark:focus:ring-gray-800
        transition-all duration-200"
    />
  </div>
);

export { CheckBox, InputField, RadioBtn };
